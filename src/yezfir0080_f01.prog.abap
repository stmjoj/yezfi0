...
FORM CALL_SCREEN_0100 .
  ...
  YEZFIS0170-BUKRS     = P_BUKRS.
  YEZFIS0170-BUTXT     = P_BUTXT.
  YEZFIS0170-DTCNT     = LINES( GT_OUTTAB[] ).
...
*----------------------------------------------------------------------*
  IF ( P_NORM IS NOT INITIAL ).              " 일반항목
    PERFORM SELECT_OPEN_ITEM_BSID.

    IF ( P_APAR        IS NOT INITIAL ) AND
       ( GS_KNB1-LIFNR IS NOT INITIAL ).
      PERFORM SELECT_OPEN_ITEM_BSIK.
    ENDIF.
  ENDIF.
...
*----------------------------------------------------------------------*
  IF ( P_NORM IS NOT INITIAL ).              " 일반항목
    PERFORM SELECT_OPEN_ITEM_BSAD.

    IF ( P_APAR        IS NOT INITIAL ) AND
       ( GS_KNB1-LIFNR IS NOT INITIAL ).
      PERFORM SELECT_OPEN_ITEM_BSAK.
    ENDIF.
  ENDIF.
...
FORM SELECT_CLEARED_ITEM .
  ...
  PERFORM SELECT_CLEARED_ITEM_BSAD.

  IF ( P_APAR        IS NOT INITIAL ) AND
     ( GS_KNB1-LIFNR IS NOT INITIAL ).
    PERFORM SELECT_CLEARED_ITEM_BSAK.
  ENDIF.
...
*----------------------------------------------------------------------*
  IF ( P_NORM IS NOT INITIAL ).              " 일반항목
    PERFORM SELECT_ALL_ITEM_BSID.

    IF ( P_APAR        IS NOT INITIAL ) AND
       ( GS_KNB1-LIFNR IS NOT INITIAL ).
      PERFORM SELECT_ALL_ITEM_BSIK.
    ENDIF.
  ENDIF.
...
*----------------------------------------------------------------------*
  IF ( P_NORM IS NOT INITIAL ).              " 일반항목
    PERFORM SELECT_ALL_ITEM_BSAD.

    IF ( P_APAR        IS NOT INITIAL ) AND
       ( GS_KNB1-LIFNR IS NOT INITIAL ).
      PERFORM SELECT_ALL_ITEM_BSAK.
    ENDIF.
  ENDIF.
...
*&---------------------------------------------------------------------*
*&      Form  MAKE_KUNNR_ENTRIES
*&---------------------------------------------------------------------*
*       조회대상 고객 SELECT
*----------------------------------------------------------------------*
FORM MAKE_KUNNR_ENTRIES .

  SELECT A~KUNNR   AS KUNNR
       , B~LIFNR   AS LIFNR
    FROM KNB1 AS A INNER JOIN
         KNA1 AS B
      ON B~KUNNR =  A~KUNNR
...
FORM SELECT_OPEN_ITEM_BSIK .
  ...
  SELECT A~BUKRS     AS BUKRS
    ...
    FROM BSIK AS A INNER JOIN
         BKPF AS B
      ON B~BUKRS    =  A~BUKRS
     AND B~BELNR    =  A~BELNR
     AND B~GJAHR    =  A~GJAHR
   WHERE A~BUKRS    =  @P_BUKRS
     AND A~LIFNR    =  @GS_KNB1-LIFNR
...
*----------------------------------------------------------------------*
  ...
  LOOP AT LT_BSIK INTO DATA(LS_BSIK).
    MOVE-CORRESPONDING LS_BSIK TO GS_OUTTAB.
...
*----------------------------------------------------------------------*
  ...
  LOOP AT LT_BSID INTO DATA(LS_BSID).
    MOVE-CORRESPONDING LS_BSID TO GS_OUTTAB.
    GS_OUTTAB-STATU = ICON_LED_RED.
    APPEND GS_OUTTAB TO GT_OUTTAB.
    CLEAR GS_OUTTAB.
  ENDLOOP.
...
*----------------------------------------------------------------------*
  ...
  LOOP AT LT_BSAK INTO DATA(LS_BSAK).
    MOVE-CORRESPONDING LS_BSAK TO GS_OUTTAB.
    GS_OUTTAB-STATU = ICON_LED_RED.
    APPEND GS_OUTTAB TO GT_OUTTAB.
    CLEAR GS_OUTTAB.
  ENDLOOP.
...
*----------------------------------------------------------------------*
  ...
  LOOP AT LT_BSAD INTO DATA(LS_BSAD).
    MOVE-CORRESPONDING LS_BSAD TO GS_OUTTAB.
    GS_OUTTAB-STATU = ICON_LED_RED.
    APPEND GS_OUTTAB TO GT_OUTTAB.
    CLEAR GS_OUTTAB.
  ENDLOOP.
